...
d3.csv('./js/files/data.csv').then(data => {
  ...
  var neetData = data.filter(d => parseFloat(d.age) > 17 && parseFloat(d.age) < 35).filter(d => {
    return d.education === "Intermediate" || d.education === "Graduation" || d.education === "Masters or above";
  });
...
function createGradSankey() {
  state.currData = state.originalData.filter(d => parseFloat(d.age) > 19).filter(d => d.education === "Graduation" || d.education === "Masters or above");
  state.currTotalWeight = state.currData.reduce((a, d) => a + parseFloat(d.weight), 0);
  var res = getLabourForceGraph(state.currData);
  sankey.update(res);
}
