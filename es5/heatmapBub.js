function createPVHeatmap() {
  ...
  async function readAndDraw() {
    ...
    radScale = d3.scaleSqrt().domain([0, d3.max(everViol)]).range([0, 20]);
    ...
    distScale.range(d3.range(0, height + 1, yInterval)); //create tooltip
    ...
    var svg_g = d3.select("#vaw_heat_container").append("svg") //.attr("width", svg_width)
    ...
    .attr('fill', '#E91E63').attr('fill-opacity', 0.5).on('mousemove', function (d) {
      toolTip.createTooltip(d, d3.event);
    }).on('mouseout', function (d) {
      toolTip.removeTooltip(d);
    });
    ...
    //.attr('width', 300)
    //.attr('height', 200)
    .attr('id', 'legendSVG'); //drawCircLegend().updateCellSize(10);

    var circLegendG = d3.select('#legendSVG').append('g').attr('id', 'circLegendGroup').attr('transform', 'translate(20, 50)').call(drawCircLegend);
    var contColLegendG = d3.select('#legendSVG').append('g').attr('id', 'contColLegGroup').attr('transform', 'translate(20, 140)').call(drawContLegend);
    ...
    function drawContLegend(selection) {
      ...
      var linGrad = barG.append("defs").append("svg:linearGradient").attr("id", "gradient").attr("x1", "0%").attr("y1", "100%").attr("x2", "100%").attr("y2", "100%").attr("spreadMethod", "pad");
      linGrad.append("stop").attr("offset", "0%").attr("stop-color", '#F5F5F5').attr("stop-opacity", 1);
      linGrad.append("stop").attr("offset", "100%").attr("stop-color", "#880E4F").attr("stop-opacity", 1);
      barG.append('rect').attr('width', rectWidth).attr('height', rectHeight).attr('rx', 2).attr('ry', 2).style("fill", "url(#gradient)"); //.style('stroke', '#212121')
  ...
  function upDate(newData) {
    ...
    let rows = d3.select('#vaw_heat_container svg').select('g').selectAll('.row').data(newArrOfArr, d => d.key).transition().duration(1500).attr('transform', d => d.key == "Punjab" ? `translate(0, ${distScale(d.key) + 20})` : `translate(0, ${distScale(d.key)})`);
  }
