...
d3.csv('./js/files/data.csv').then(data => {
  ...
  var employedData = data.filter(d => d.employment === 'employed');
  state.currData = employedData;
  state.currTotalWeight = state.currData.reduce((a, d) => a + parseFloat(d.weight), 0); //res = buildGraph(data, ['employment', 'empType','industry', 'barrier']);

  var res = buildGraph(state.currData, ['employment', 'empType', 'industry', 'barrier']); //res = getLabourForceGraph(data);
...
function buildGraph(data, colArr) {
  ...
  var nodesArr = nodes.reduce((a, d, i) => {
    return a.concat(Object.keys(d).map(d => {
      return {
        name: d,
        col: colArr[i]
      };
  ...
  var nodesDict = {};
  nodesArr.forEach((d, i) => {
    nodesDict[d.name + '-' + d.col] = i;
  });
  var edgesArr = [];
...
function getLabourForceGraph(data) {
  ...
  var columnLinks = [['laborForce', 'employment'], ['laborForce', 'barrier'], ['employment', 'barrier'], ['employment', 'industry'], ['industry', 'barrier']];
  ...
  addNodesToGraph(industryVals, 'industry', graph); //add links
    ...
    columnLinks.forEach(cl => {
      ...
      if (linksObj) {
        ...
        if (linkVal !== undefined) {
          linksObj[row[target] + '-' + target] = linksObj[row[target] + '-' + target] + parseFloat(row.weight);
        }
      }
    });
  });
