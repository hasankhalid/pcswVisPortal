function drawBars() {
  ...
  var chartWidth = 400,
      ...
      gapBetweenGroups = 10,
      spaceForLabels = 100,
      spaceForLegend = 225; // Zip the series data together (first values, second values, etc.)
  ...
  var x = d3.scaleLinear().domain([0, d3.max(zippedData)]).range([0, chartWidth]);
  var y = d3.scaleLinear().range([chartHeight + gapBetweenGroups, 0]);
  var yAxis = d3.axisLeft().scale(y).tickFormat('').tickSize(0); // Specify the chart area and dimensions

  var chart = d3.select(".chart") //  .attr("width", spaceForLabels + chartWidth + spaceForLegend)
  //  .attr("height", chartHeight)
  .attr("viewBox", "0 0 " + (spaceForLabels + chartWidth + spaceForLegend) + " " + chartHeight).attr("preserveAspectRatio", "xMinYMin meet"); // Create bars
  ...
  }).attr("class", "bar").attr("width", x).attr("height", barHeight - 1); // Add text label in bar
  ...
  chart.append("g").attr("class", "y axis").attr("transform", "translate(" + spaceForLabels + ", " + -gapBetweenGroups / 2 + ")").call(yAxis); // Draw legend
