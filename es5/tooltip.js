...
function Tooltip({
  ...
  function createTooltip(d, event, i) {
    ...
    if (!tooltipElement) {
      ...
      currTooltip = tooltip;
      tooltip.attr('id', idPrefix + d[dataId]).style('position', 'fixed').style('opacity', 0);
      stylingFunc(tooltip, d);
      d3.select('body').append(function () {
        return tooltip.node();
      });
      let finalPos = getTooltipPosition(event, tooltip.node());
      tooltip.style('z-index', 10000).style('left', finalPos[0] + 'px').style('top', finalPos[1] + 'px').transition().duration(300).style('opacity', 1);
  ...
  function removeTooltip(d) {
    ...
    if (tooltip) {
      d3.select(tooltip).transition().duration(100).style('opacity', 0).on('end', function () {
        currTooltip = null;
      }).remove();
    }
  ...
  function touchListener(e) {
    if (currTooltip) {
      currTooltip.transition().duration(100).style('opacity', 0).on('end', function () {
        currTooltip = null;
      }).remove();
      window.removeEventListener('touchend', touchListener);
    }
