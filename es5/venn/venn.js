...
(function (venn) {
  ...
  venn.VennDiagram = function () {
    var width = 800,
        ...
        styled = true,
        fontSize = null,
        colours = d3v3.scale.ordinal().range(["#0a2756", "#e60024", "#45CFD7", "#a173d1", "#0FF66A3", "#00FFFF", "#6ab975", "#5687d1", "#F5E94B", "bbbbbb", "#00B8E6"]),
    ...
    function chart(selection) {
      ...
      var textCentres = computeTextCentres(circles, data); // draw out a svg
      ...
      svg.attr('viewBox', '0 0 ' + width + ' ' + height); // to properly transition intersection areas, we need the
      // previous circles locations. load from elements

      var previous = {},
          hasPrevious = false;
      svg.selectAll("g").each(function (d) {
        var path = d3v3.select(this).select("path").attr("d");
      ...
      update.select("path").attrTween("d", pathTween);
      var updateText = update.select("text").text(function (d) {
        return label(d);
      }).attr("x", function (d) {
      ...
      exit.select("path").attrTween("d", pathTween);
      var exitText = exit.select("text").text(function (d) {
        return label(d);
      }).attr("x", width / 2).attr("y", height / 2); // if we've been passed a fontSize explicitly, use it to
      // transition
  ...
  function computeTextCentre(interior, exterior) {
    ...
    var solution = venn.fmin(function (p) {
  ...
  venn.computeTextCentre = computeTextCentre; // given a dictionary of {setid : circle}, returns
  ...
  venn.computeTextCentres = computeTextCentres; // sorts all areas in the venn diagram, so that
...
(function (venn) {
  ...
  venn.venn = function (areas, parameters) {
    ...
    var fmin = parameters.fmin || venn.fmin; // add in missing pairwise areas as having 0 size

    areas = addMissingAreas(areas); // initial layout is done greedily

    var circles = initialLayout(areas); // transform x/y coordinates to a vector to optimize
    ...
    var solution = fmin(function (values) {
  ...
  venn.getDistanceMatrices = function (areas, sets, setids) {
    ...
    var distances = venn.zerosM(sets.length, sets.length),
        constraints = venn.zerosM(sets.length, sets.length); // compute required distances between all the sets such that
    // the areas match
  ...
  venn.bestInitialLayout = function (areas, params) {
    var initial = venn.greedyLayout(areas, params); // greedylayout is sufficient for all 2/3 circle cases. try out
    ...
    for (i = 1; i < mostOverlapped.length; ++i) {
      ...
      for (var j = 0; j < overlap.length; ++j) {
        ...
        var p1 = circles[overlap[j].set],
            d1 = venn.distanceFromIntersectArea(set.radius, p1.radius, overlap[j].size); // sample positions at 90 degrees for maximum aesthetics
        ...
        for (var k = j + 1; k < overlap.length; ++k) {
          var p2 = circles[overlap[k].set],
              d2 = venn.distanceFromIntersectArea(set.radius, p2.radius, overlap[k].size);
  ...
  venn.classicMDSLayout = function (areas) {
    ...
    var positions = mds.classic(distances); // translate rows back to (x,y,radius) coordinates
  ...
  venn.lossFunction = function (sets, overlaps) {
    ...
    for (var i = 0; i < overlaps.length; ++i) {
      ...
      if (area.sets.length == 1) {
        ...
        overlap = venn.circleOverlap(left.radius, right.radius, venn.distance(left, right));
      } else {
  ...
  venn.normalizeSolution = function (solution, orientation) {
    orientation = orientation || Math.PI / 2; // work with a list instead of a dictionary, and take a copy so we
    // don't mutate input
    ...
    var clusters = venn.disjointCluster(circles); // orientate all disjoint sets, get sizes
...
(function (venn) {
  ...
  var c1 = 1e-6,
      c2 = 0.1; /// searches along line 'pk' for a point that satifies the wolfe conditions
...
(function (venn) {
  ...
  function getIntersectionPoints(circles) {
    ...
    for (var i = 0; i < circles.length; ++i) {
      for (var j = i + 1; j < circles.length; ++j) {
        var intersect = venn.circleCircleIntersection(circles[i], circles[j]);
