function createEmpTree() {
  let labHierData;
  let minRadius = 0;
  let maxRadius = 300; // define Scales

  let radScale = d3.scaleSqrt().domain([0, 100]).range([minRadius, maxRadius]);
  let colScale = d3.scaleOrdinal(); // define dimensions
  ...
  let margin = {
    ...
    left: 180
      ...
      height = 1020 - margin.top - margin.bottom; // define counter, duration and root

  let i = 0,
      duration = 500,
      root;
  let uniqueEdu, uniqueInd; // define tree and diagonals;
  ...
  let svg = d3.select("#tree_container").append("svg") // .attr("width", width + margin.right + margin.left)
  // .attr("height", height + margin.top + margin.bottom)
  .attr("preserveAspectRatio", "xMinYMin meet").attr("viewBox", "0 0 " + width + " " + (height + margin.top + margin.bottom)) //.attr("viewBox", "0 0 " + (width + margin.right + margin.left) + " 2300")
  .append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  svg.selectAll('text.layerTitle').data(["Employed", "Area", "Education", "Industry"]).enter().append('text').text(d => d).attr('x', (d, i) => i * 225).attr('y', -20).style('text-anchor', 'middle').style('font-size', '20px').style('text-decoration', 'underline'); //.style('font-weight', 'bold');
  ...
  async function readDataAndDraw() {
    ...
    let colors = ['grey', '#4db6ac', '#29b6f6', '#1b9e77', '#d95f02', '#7570b3', '#e7298a', '#66a61e', '#e6ab02', '#a6761d']; // set colScale domain and range

    colScale.domain(categs).range(colors);
    let seqData = preProcess(data);
    let seqSums = d3.nest().key(function (d) {
      return d.sequence;
    }).rollup(v => d3.sum(v, row => row.weight)).entries(seqData);
    let sumNodes = d3.sum(seqSums, d => d.value);
    radScale.domain([0, sumNodes]);
    let seqSumsAoA = convArrOfArr(seqSums);
    let seqSumJSON = buildHierarchy(seqSumsAoA, "Employed"); //radScale.domain()
  ...
  function update(source) {
    ...
    nodeEnter.append("circle").attr("r", 1).style('fill', '#a6cee3').style("fill-opacity", d => d.data.name != "Employed" ? 0.75 : 0);
    ...
    .style("fill", d => d._children || d.children ? colScale(d.data.name) : colScale(d.parent.data.name)).style("fill-opacity", d => d.data.name != "Employed" ? 0.75 : 0);
    ...
    var linkEnter = link.enter().insert("path", "g") //.append('path')
    //.attr("class", "link")
    .attr("d", function (d) {
      var o = {
        x: source.x0,
    ...
    }).style('stroke-width', d => d.target.Perc).classed('link', true).attr('id', d => `ID${d.target.id}`); // Transition links to their new position.

    link.merge(linkEnter).transition().duration(duration).attr("d", diagonal).style('stroke-width', d => 2 * radScale(d.target.Perc)).style("stroke", d => d.target._children || d.target.children ? colScale(d.target.data.name) : colScale(d.target.parent.data.name));
  ...
  function mouseO(over) {
    return function (d) {
      ...
      if (over == true) {
        ...
        d3.select('body').append('div').classed('animated', true).classed('fadeInOpac', true).classed('tool', true).attr('id', 'hoverbox'); // tooltip selection

        var tooltip = d3.select('.tool');
        tooltip.append('div').classed('colorTypeBar', true).style('border-color', colScale(d.data.name)).html(function () {
          return '<p class="no_margin">Entire Sequence: <span class="percentage">' + percentage + ' %</span></p>';
        });

        if (d.parent) {
          tooltip.append('div').classed('colorTypeBar', true).style('border-color', colScale(d.parent.data.name)).html(function () {
            return '<p class="no_margin">Relative to Parent: <span class="percentage">' + percentageParent + ' %</span></p>';
          });
        }

        tooltip.append('div').classed('text_tool', true).html(function () {
          if (d.layer === "Industry") {
            return '<p class="p_margin"><i class="fas fa-info-circle"></i> Last node. Click parent to collapse.</p>';
          } else {
            let expCon;
